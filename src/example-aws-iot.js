...
ServerActions.RegisterIO('socket-io', require('socket.io-client'));
    ...
    cognitoUser.authenticateUser(authenticationDetails, {
      onSuccess: function (result) {
        userPool.getCurrentUser().getSession((err, session) => {
          ...
          let providerKey = `cognito-idp.${process.env.AWS_REGION}.amazonaws.com/${process.env.AWS_COGNITO_USERPOOL_ID}`;
...
.then((result) => {
    ...
    return Login(result.username, result.password);
})
.then(user => {
  ...
  var Flux = new Fluxxor.Flux(stores, ServerActions.Actions);
